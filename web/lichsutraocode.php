...
if (isset($_SESSION['id']) && isset($_SESSION['username'])) {
    ...
    <div class="container-fluid">
        <div class="card">
            <div class="card-body">
                <h5 class="card-title fw-semibold mb-4">Lịch sử trao code</h5>
                <div class="card">
                    <div class="card-body">
                        <table class="table">
                            ...
                            $query = "SELECT g.code, g.count, g.time_expire, g.trangthai, u.username
                                        FROM giftcodett g INNER JOIN user u ON g.code = u.gitcodetrao
                                        WHERE u.id = '$id'";
                            ...
                            if (mysqli_num_rows($result) > 0) {
                                ...
                                while ($row = mysqli_fetch_assoc($result)) {
                                    ...
                                    if($row["count"] == 1)
                                    {
                                        $content .= '<td><a style="color: red"><i class="fas fa-check-circle fa-lg fa-spin"></i> Chưa nhập</a></td>';
                                    }
                                    else
                                    {
                                        $content .= '<td><a style="color: green"><i class="fas fa-check-circle fa-lg fa-spin"></i> Đã nhập</a></td>';
                                    }
                                    $content .= "</tr>";
                                    $count++;
                                }                                
    ...
    $query = "SELECT COUNT(*) AS totalRows FROM giftcodett g INNER JOIN user u ON g.code = u.gitcodetrao WHERE u.id = '$id'";
    $result = mysqli_query($conn, $query);
