...
namespace TienKiemV2Remastered.Application.Handlers.Character
{
    public class DiscipleHandler : ICharacterHandler
    {
                ...
                case 1:
                    {
                        ...
                        {
                            if (Disciple.InfoChar.Stamina <= 0 && timeServer > Disciple.InfoDelayDisciple.AutoChat)
                            {
                                ...
                                Disciple.InfoDelayDisciple.AutoChat += 10000 + timeServer;
                            }
                            else if (Disciple.InfoChar.Mp <= (Disciple.InfoChar.OriginalMp * 5 / 100) && timeServer > Disciple.InfoDelayDisciple.AutoChat)
                            {
        ...
        private void HandleUseSkill(bool isAuto = true, int charId = -1, int modId = -1)
        {
            ...
            {
                if (Disciple.Zone.ZoneHandler.GetCharacter(Disciple.CharacterFocus.Id) == null)
                {
                ...
                try
                {
                    ...
                    var enemyDistance = Math.Abs(enemy.InfoChar.X - Disciple.InfoChar.X);
                    var enemyDistanceY = Math.Abs(enemy.InfoChar.Y     - Disciple.InfoChar.Y);
                    ...
                    for (int i = Disciple.Skills.Count - 1; i >= 0; i--)
                    {
                        ...
                        {
                            if (i == 1 && ((enemyDistance > 44 && enemyDistance <= dX) || (enemyDistanceY > 44 && enemyDistanceY <= dY)))
                            {
                                break;
                            }
                            else if (i == 0)
                            {
                                if ((enemyDistance <= dX && enemyDistanceY <= dY) || Disciple.Skills.Count == 1)
                                {
                    ...
                    //if (skillChar.Id == 24)
                    //{
                    //    SkillHandler.SkillNotFocus(Disciple, skillChar.Id, 10);
                    //    return;
                    //}
                    //// 
                    //if (skillChar.Id == 25)
                    //{
                    //    SkillHandler.SkillNotFocus(Disciple, skillChar.Id, 11);
                    //    return;
                    //}
                    //// 
                    //if (skillChar.Id == 26)
                    //{
                    //    SkillHandler.SkillNotFocus(Disciple, skillChar.Id, 12);
                    //    return;
                    //}
                    if (!enemy.InfoChar.IsDie)
                    {
                        var listPlayer = new List<ICharacter> { enemy };
                        var skillTemplate = Cache.Gi().SKILL_TEMPLATES.FirstOrDefault(sk => sk.Id == skillChar.Id);
                        var skillDataTemplate = skillTemplate?.SkillDataTemplates.FirstOrDefault(so => so.SkillId == skillChar.SkillId);
        ...
        private void AutoPlusPoint(long timeServer)
        {
            if (Disciple.InfoDelayDisciple.AutoPlusPoint <= timeServer && Disciple.PlusPoint.PointNext <= Disciple.InfoChar.Potential)
            {
                ...
                {  
                    ...
                    SetUpInfo();
                    Disciple.InfoDelayDisciple.AutoPlusPoint = 100 + timeServer;
                }
        ...
        private void AutoAddSkill()
        {
            switch (Disciple.InfoChar.Power)
            {
                ...
                case >= 80000000000 when Disciple.Skills.Count < 5:
                    {
                        var randomSkill = DataCache.IdSkillDisciple5[ServerUtils.RandomNumber(DataCache.IdSkillDisciple5.Count)];
                        Disciple.Skills.Add(new SkillCharacter()
                        {
                            Id = randomSkill,
                            SkillId = Disciple.GetSkillId(randomSkill),
        ...
        public void SetInfoSet()
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                if (Disciple.ItemBody[i] == null || ItemCache.ItemTemplate(Disciple.ItemBody[i].Id).Level != 14)
                {
        ...
        public void SetEnhancedOption()
        {
            ...
            Disciple.InfoOption.PhanTramHutHP += GetParamItem(95);

            Disciple.InfoOption.PhanTramHutKI += GetParamItem(96);
            Disciple.InfoOption.PhanTramSatThuongChiMang += GetParamItem(5);
            Disciple.InfoOption.VoHieuHoaChuong += GetParamItem(3);
        }
        ...
        public void PlusTiemNang(IMonster monster, int damage)
        {
            ...
            if (Disciple.InfoSet.IsFullSetThienXinHang || Disciple.InfoSet.IsFullSetCadic || Disciple.InfoSet.IsFullSetOcTieu)
            {
                damagePlusPoint *= 2;   // giảm 10 xg 2
            }
           // Đệ ma bư up sức mạnh lâu hơn đệ tử thường
            ...
            if (damagePlusPoint >= 100000000)// maxx 100tr tnsm 1 hit
            {
                damagePlusPoint = 100000000;
            }
            if (Disciple.Character.ItemBody[5] != null)
            {
                var xtnsm = Disciple.Character.ItemBody[5].Options.FirstOrDefault(i => i.Id == 160);
                if (xtnsm != null)
                {
                    damagePlusPoint += damagePlusPoint * xtnsm.Param / 100;
                }
            }
        ...
        {
            // if (!Disciple.Character.InfoChar.IsPremium && Disciple.Character.InfoChar.Điểm_thành_tích >= DataCache.PREMIUM_LIMIT_UP_POWER)
            // {
